  ...
  if (req.isAuthenticated()) {
    pool.connect(function(errorConnectingToDb, db, done) {
      if (errorConnectingToDb) {
        ...
        db.query('UPDATE "users" SET "fname"=$1, "lname"=$2, "email"=$3, "role"=$4, "session_count"=$5, "year"=$6 WHERE "id" = $7;', [body.fname, body.lname, body.email, body.role, session_count, year, id],
          function(queryError, result) {
            done();
            if (queryError) {
              res.sendStatus(500);
